...
Expert Linux administration skill that **implements fixes** for issues found by `my-bpm-linux-audit`. Reads open issues from the host's tracking repo (`bpm-{hostname}`), triages them by severity, and assigns them to specialist teammates for implementation.

Runs as an **agent team** with Team Lead (delegate mode), admin teammates, and Codex review gates. Follows the same orchestration pattern as `my-bpm-experteam-openissues`.

**This skill IMPLEMENTS — it does NOT audit.** Use `my-bpm-linux-audit` first to create findings, then this skill to fix them.
