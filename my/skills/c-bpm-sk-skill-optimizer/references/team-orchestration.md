...
- List all `c-bpm-sk-` prefixed skills (custom)
- List all non-prefixed skills (originals)
- Identify skills without `c-bpm-sk-` counterparts (candidates for forking)
- Identify orphaned `c-bpm-sk-` skills (originals removed)
...
|-----------|-----------------|
| **Segregation** | Is the original untouched? Does the `c-bpm-sk-` version exist? |
...
- `skill-forker` — Forking originals to `c-bpm-sk-` versions
...
4. **Segregation reminder** — Originals are READ ONLY, all changes go to `c-bpm-sk-` versions
5. **Skills to load** — `c-bpm-sk-skill-optimizer` for naming/workflow conventions, `skill-creator` for creation process
...
- **Skill name** — With `c-bpm-sk-` prefix
...
- **Load `skill-creator`** for creation guidance and `c-bpm-sk-skill-optimizer` for custom rules
...
| `c-bpm-sk-` prefix | Directory and frontmatter `name:` both use `c-bpm-sk-` prefix |
