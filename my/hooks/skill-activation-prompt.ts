...
/**
 * Word-boundary keyword matching that handles punctuation-heavy terms
 * like .env, curl|bash, hyphenated-names correctly.
 */
function matchKeyword(prompt: string, kw: string): boolean {
  const escaped = kw.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  // Punctuation-safe boundaries: match at start/end of string, whitespace, or non-word chars
...
async function main() {
    try {
        ...
        for (const [skillName, config] of Object.entries(rules.skills)) {
            ...
            // Intent pattern matching FIRST (strong signal, may trigger block)
            if (triggers.intentPatterns) {
                ...
                if (intentMatch) {
                    ...
                    continue; // Intent match is stronger than keyword — skip keyword check
                }
            }

            // Keyword matching (weaker signal, never triggers block)
            if (triggers.keywords) {
                const matchedKeyword = triggers.keywords.find(kw =>
                    matchKeyword(prompt, kw.toLowerCase())
                );
                if (matchedKeyword) {
            ...
            for (const [agentName, config] of Object.entries(rules.agents)) {
                ...
                if (triggers.intentPatterns) {
                    ...
                    if (intentMatch) {
                        ...
                        continue; // Intent match is stronger — skip keyword check
                    }
                }
        ...
        if (matchedSkills.length > 0 || matchedAgents.length > 0 || patternInference || semanticQuery.isSemanticQuery) {
            ...
            if (matchedSkills.length > 0 || matchedAgents.length > 0) {
                ...
                const blockingSkills = matchedSkills.filter(s => 
                    s.config.enforcement === 'block' && s.matchType === 'intent'
                );
