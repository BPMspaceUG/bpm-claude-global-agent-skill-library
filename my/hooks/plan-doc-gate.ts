// plan-doc-gate.ts — TypeScript source for dist/plan-doc-gate.mjs.
//
// PreToolUse hook for BPMspaceUG#105. Enforces #104 at RUNTIME: everything
// belongs in the GitHub Issue, so an agent must not author plan/doc side-car
// files (`~/.claude/plans/*.md`, scratchpad `prompt.md`, `plan.md`,
// `ISSUE_<n>_PLAN.md`, ...).
//
// NO BUILD STEP EXISTS in this repo. dist/plan-doc-gate.mjs is the
...
const MAINTAINED_DOCS: ReadonlySet<string> = new Set([
...
const ALLOW_SEGMENTS: ReadonlySet<string> = new Set([
...
// basename, lowercased).
const SIDECAR_STEMS: ReadonlySet<string> = new Set([
...
const deny  = (r: string): never => emit('deny', `[plan-doc-gate] ${r}`);
...
function classify(rawPath: string): string | null {
